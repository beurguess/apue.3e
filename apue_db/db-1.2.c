...
#include <stdarg.h>
#include <errno.h>
#include <sys/uio.h>	/* struct iover */
...
/*
 * The fllowing definitions are for hash chains and free
...
/* 
 * the table size change to set while call db_open.
 */
...
typedef struct {
	...
	COUNT 	cnt_stor5;	/* store: DB_INSERT, use bigger, reused */
...
static int 	_db_bigfree(DB *, const char *, const char *);
static void 	_db_doblank(char *);

/*
 * Open or create a database. Same arguments as open(2).
 * One new arguments for hash table size.
 */
DBHANDLE db_open(const char *pathname, int oflag, ...)
{
	...
	if (oflag & O_CREAT) {
		...
		va_start(ap, oflag);
		mode = va_arg(ap, int);
		db->nhash = va_arg(ap, int); 		/* hash table size */
		va_end(ap);
		...
		nhash = 0;
		for (;;) {
			if ((i = read(db->idxfd, asciiptr, PTR_SZ)) < 0)
				err_sys("db_open: read error from index record");
			if (asciiptr[0] == '\n')
				break;
			nhash++;
		}
	...
	if ((oflag & (O_CREAT | O_TRUNC)) == (O_CREAT | O_TRUNC)) {
		...
		char	hash[(db->nhash + 1) * PTR_SZ + 2];	 /* +2 for newline and null */
...
static DB *_db_alloc(int namelen)
{
	 ...
	 * +5 for ".idx" ro ".dat" plus null at end.
	 */
 ...
 * may point to. Also close the descriptors if still open.
 */
...
static int _db_find_and_lock(DB *db, const char *key, int writelock)
{
	...
	while (offset != 0) {
		...
		strcpy(realkey, db->idxbuf);
		if (use_big_hash)
			_db_doblank(realkey);
		if (strcmp(realkey, key) == 0)
			break;	/* found a match */
		db->ptroff = offset;	/* offset of this (unequal) record */
		offset = nextoffset;	/* next one of compare */
	}
	/*
...
static off_t _db_readidx(DB *db, off_t offset)
{
	...
	if ((db->idxlen = atol(asciilen)) < IDXLEN_MIN ||
	...
	/*
	 * Find the separetors in the index record.
	 */
...
static void _db_dodelete(DB *db)
{
	...
	*ptr = 0; 	/* null terminate for _db_writedat */
	 ...
	 * the deleted record becomes the head or the free list.
	 */
	...
	/*
	 * Rewrite the chain ptr tha pointed to this record being
...
static void _db_writedat(DB *db, const char *data, off_t offset, int whence)
{
	...
	/*
	 * If we're appending, we here to lock before doing the lseek
	...
	if (writev(db->datfd, &iov[0], 2) != db->datlen)
		err_dump("_db_writedat: writev error for data record");
...
static void _db_writeptr(DB *db, off_t offset, off_t ptrval)
{
	...
	if (ptrval < 0 || ptrval > PTR_MAX)
		err_quit("_db_writeptr: invaild ptr: %d", ptrval);
...
int db_store(DBHANDLE h, const char *key, const char *data, int flag)
{
	...
	if (datlen < DATLEN_MIN || datlen > DATLEN_MAX)
		err_dump("db_store: invaild data length");
	...
	if (_db_find_and_lock(db, key, 1) < 0) {	/* record not found */
		...
		if (_db_findfree(db, keylen, datlen) < 0) {
			if (use_big_hash && _db_bigfree(db, data, key) != -1) {
				/*
				 * Used bigger hash chain.
				 */
				db->cnt_stor5++;
			} else {
				/*
		...
		if (datlen != db->datlen) {
			 ...
			 * (it way change with the deletion).
			 */
			ptrval = _db_readptr(db, db->chainoff);
			...
			/*
			 * Same size date, just replace data record.
			 */
			_db_writedat(db, data, db->datoff, SEEK_SET);
			db->cnt_stor4++;
		}
...
doreturn:	/* unlock hash chain locked bu _db_find_and_lock */
	if (un_lock(db->idxfd, db->chainoff, SEEK_SET, 1) < 0)
...
static int _db_bigfree(DB *db, const char *data, const char *key)
{
	char 	*pdat, *pkey;
	int 	datlen = strlen(data);
	int 	keylen = strlen(key);
	int 	rc;
	int 	saveoffset, nextoffset, offset, ptrval;

	/* lock the free hash chain */
	if (writew_lock(db->idxfd, FREE_OFF, SEEK_SET, 1) < 0)
		err_dump("_db_bigfree: writew_lock error");
	...
	if (offset == 0) {
		...
		_db_writeptr(db, saveoffset, db->ptrval);
		int idxlen = strlen(db->idxbuf);
		if ((pdat = malloc(db->datlen+1)) == NULL ||
				(pkey = malloc(idxlen+1)) == NULL)
			err_dump("_db_bigfree: malloc error");
		memset(pdat, ' ', db->datlen);
		pdat[db->datlen] = 0;
		memset(pkey, ' ', idxlen);
		pkey[idxlen] = 0;
		strcpy(pdat, data);
		pdat[datlen] = ' ';
		strcpy(&pkey[idxlen-keylen], key);
		_db_writedat(db, pdat, db->datoff, SEEK_SET);
		_db_writeidx(db, pkey, db->idxoff, SEEK_SET, ptrval);
		_db_writeptr(db, db->chainoff, db->idxoff);
		free(pdat);
		free(pkey);
		rc = 0;
	...
	if (un_lock(db->idxfd, FREE_OFF, SEEK_SET, 1) < 0)
		err_dump("_db_bigfree: un_lock error");
	return(rc);
}

static void _db_doblank(char *key)
{
	char 	buf[IDXLEN_MAX];
	int 	i, n;
